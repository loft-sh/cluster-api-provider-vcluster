...
IMG ?= tylergillson/cluster-api-provider-vcluster:$(TAG)
...
# revert the values back to development ones 
	sed -i'' -e 's@image: .*@image: tylergillson/cluster-api-provider-vcluster:main@' ./config/default/manager_image_patch.yaml
